...
namespace BucketStrategyLibrary
{
    ...
    public class BucketStrategy
    {
        ...
        public async Task MakeSell(string ticker)
        {
            // Get most recent days of data
            List<GeneralStockData> data = await IEXDataAccess.GetGeneralData("5d", ticker);

            // make sell in database
            double sellValue = data[data.Count() - 1].open;
            string sellDate  = DateTime.Today.Month.ToString() + "/" + DateTime.Today.Day.ToString() + "/" + DateTime.Today.Year.ToString();

            SQLiteAccess.MakeSell(m_strategyName, ticker, sellDate, sellValue);
        }

        public async Task MakeBuy(string ticker, int bucketNumber)
        {
            // get the most recent window of data
            List<GeneralStockData> data = await IEXDataAccess.GetGeneralData(m_dataTimeFrame, ticker);

            List<double> window = new List<double>();
            for (int iii = data.Count() - durationToInt(m_slidingWindowSize); iii < data.Count(); ++iii)
                window.Add(data[iii].open);

            double buyValue = window[window.Count() - 1];
            ...
            normalize(ref window);

            // compute the similarity value between the aggregate and the window of data
            double similarityVal = m_categories[bucketNumber].ComputeSimilarity(window);

            // if the similarity value is above the threshold value, generate a BUY by recording the following information
            // Ticker, Date, Similarity Value, Price, % Return on the future return date
            if (similarityVal >= m_similarityThreshold)
            {
                string buyDate = DateTime.Today.Month.ToString() + "/" + DateTime.Today.Day.ToString() + "/" + DateTime.Today.Year.ToString();

                SQLiteAccess.MakeBuy(m_strategyName, ticker, buyDate, buyValue);
            }
